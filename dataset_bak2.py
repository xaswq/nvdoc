...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 使用新的resize函数
        new_h,new_w = resize_keep_aspect_ratio(im) 
        ...
        bm[0] = (bm[0] - l - 1.01) / (448.0 - l - r) * (new_w - 2)
        bm[1] = (bm[1] - t - 1.01) / (448.0 - t - b) * (new_h - 2)
        
        bmm = (bm[0] >= 0) & (bm[1] >= 0) & (bm[0] <= new_w-1) & (bm[1] <= new_h-1)
        [y, x] = bmm.nonzero()
        minx, maxx, miny, maxy = min(x), max(x), min(y), max(y)
        ...
        bm0 = cv2.resize(bm00, (new_w, new_h))
        bm1 = cv2.resize(bm11, (new_w, new_h))
